...
Nonetheless, the probability plot (Fig.~\ref{fig:pp}, right panel) shows that all eight
GLOF-masked annual maxima lie above the reference diagonal, indicating a consistent
tendency toward higher-than-expected peaks even after removing the direct GLOF
contribution. This pattern could reflect gradual hydroclimatic change during the 2011--2018
period --- for instance, increasing precipitation intensity under a warmer climate --- that
is superimposed on the natural reference variability. With only $n = 8$ observations,
however, neither the KS test nor the Rank-Sum test can distinguish this systematic
elevation from sampling variability, and $H_0^{(2)}$ cannot be rejected at $\alpha =
0.05$. A longer post-bypass record is needed to determine whether this upward tendency
represents a persistent hydroclimatic trend.
