...
Table~\ref{tab:ks} reports the KS statistics ($n = 8$, $C_{0.05} = 0.455$). The GLOF-inclusive peaks
reject $H_0^{(1)}$ in both datasets (BAFU~2219: $D_n = 0.610$, $p = 0.002$;
GRDC: $D_n = 0.776$, $p < 0.001$). The GLOF-masked peaks fail to reject $H_0^{(2)}$ in both
($D_n < 0.37$, $p > 0.19$).
...
\begin{table}[H]
  ...
  \begin{tabular}{llcccl}
    ...
    \multirow{2}{*}{BAFU 2219 (daily max.)}
      & GLOF-inclusive peaks & 0.610 & 0.455 & 0.002    & Reject $H_0^{(1)}$ at 5\% \\
      & GLOF-masked peaks    & 0.360 & 0.455 & 0.195    & Fail to reject $H_0^{(2)}$ \\
    \midrule
    \multirow{2}{*}{GRDC (daily mean)}
      & GLOF-inclusive peaks & 0.776 & 0.455 & $<$0.001 & Reject $H_0^{(1)}$ at 5\% \\
      & GLOF-masked peaks    & 0.318 & 0.455 & 0.321    & Fail to reject $H_0^{(2)}$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
Table~\ref{tab:ranksum} shows the Rank-Sum results. Comparison~5a rejects $H_0^{(1)}$ in both
datasets. Comparison~5b fails to reject $H_0^{(2)}$, confirming masking restores homogeneity.
Comparison~5c (pre-GLOF reference vs.\ GLOF-masked peaks) also fails to reject
(BAFU~2219: $p = 0.172$; GRDC: $p = 0.233$).

\begin{table}[H]
  \centering
  \caption{Rank-Sum test results (two-sided, $\alpha = 0.05$) for both discharge
           datasets. Comparison~5c tests whether the GLOF-masked peaks are consistent
  ...
  \begin{tabular}{lllccl}
    ...
    \multirow{3}{*}{BAFU 2219}
      & 5a & GLOF-inclusive peaks vs.\ non-GLOF years  & 499.0 & 0.002 & Reject $H_0^{(1)}$ \\
      & 5b & GLOF-masked vs.\ non-GLOF years           & 382.0 & 0.182 & Fail to reject $H_0^{(2)}$ \\
      & 5c & Pre-GLOF reference vs.\ GLOF-masked peaks  & 188.0 & 0.172 & Fail to reject \\
    \midrule
    \multirow{3}{*}{GRDC}
      & 5a & GLOF-inclusive peaks vs.\ non-GLOF years        & 486.0 & 0.001 & Reject $H_0^{(1)}$ \\
      & 5b & GLOF-masked vs.\ non-GLOF years                & 349.0 & 0.226 & Fail to reject $H_0^{(2)}$ \\
      & 5c & Pre-GLOF reference vs.\ GLOF-masked peaks & 198.0 & 0.233 & Fail to reject \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{Left: 2011--2018 BAFU~2219 daily-maximum peaks on the reference GEV
           ...
           (Module~1, \S3.2). GLOF-inclusive peaks lie above the diagonal almost entirely
           owing to 2018; GLOF-masked peaks are broadly consistent with the 1:1 line.}
  \label{fig:pp}
\end{figure}
