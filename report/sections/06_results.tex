...
$\hat{\xi}$ by $+0.091$ relative to the reference for BAFU~2219 and by $+0.069$ for
GRDC; masking reduces this residual shift to $+0.043$ and $-0.008$ respectively,
recovering near-reference tail behaviour. The AIC is substantially lower for the masked
than the full-series fit.
...
and~\ref{tab:quantiles_grdc}. The full AMS 100-year quantile exceeds the GLOF-masked
estimate by \SI{15.0}{\percent} for BAFU~2219 and \SI{15.3}{\percent} for GRDC, growing
...
\begin{figure}[H]
  ...
  \caption{Return period plot for BAFU~2219 extended AMS (1944--2025): full (blue),
           ...
           confidence bands. Black crosses ($\times$) mark GLOF-year annual maxima from
           the full series; green triangles ($\blacktriangle$) mark the corresponding
           masked values, with year labels for both. The 2018 event at
           $\hat{T} \approx 789$\,yr dominates the full-series upper tail; after masking
           it drops to $\hat{T} \approx 1.5$\,yr (green triangle near the left).
           The GLOF-masked curve lies slightly above the reference at all return periods,
           and the masked GLOF-year observations (green triangles) tend to plot above
           the reference GEV, suggesting a modest upward tendency even without direct
           GLOF contributions. This shift is not statistically significant: the KS test
           ($p = 0.195$) and Rank-Sum comparison~5b ($p = 0.172$) both fail to reject
           $H_0^{(2)}$ at $\alpha = 0.05$.}
  \label{fig:gev_three}
\end{figure}
...
Table~\ref{tab:ks} reports the KS statistics ($n = 8$, $C_{0.05} = 0.455$). The GLOF-inclusive
reject $H_0^{(1)}$ in both datasets (BAFU~2219: $D_n = 0.610$, $p = 0.002$;
GRDC: $D_n = 0.776$, $p < 0.001$). The GLOF-masked fail to reject $H_0^{(2)}$ in both
($D_n < 0.37$, $p > 0.19$).
...
\begin{table}[H]
  ...
  \begin{tabular}{llcccl}
    ...
    \multirow{2}{*}{BAFU 2219 (daily max.)}
      & GLOF-inclusive & 0.610 & 0.455 & 0.002    & Reject $H_0^{(1)}$ at 5\% \\
      & GLOF-masked    & 0.360 & 0.455 & 0.195    & Fail to reject $H_0^{(2)}$ \\
    \midrule
    \multirow{2}{*}{GRDC (daily mean)}
      & GLOF-inclusive & 0.776 & 0.455 & $<$0.001 & Reject $H_0^{(1)}$ at 5\% \\
...
Table~\ref{tab:ranksum} shows the Rank-Sum results. Comparison~5a directly tests
$H_0^{(1)}$ and rejects in both datasets (BAFU~2219: $p = 0.002$; GRDC: $p < 0.001$).
Comparison~5b directly tests $H_0^{(2)}$ and fails to reject in both (BAFU~2219:
$p = 0.172$; GRDC: $p = 0.233$). Comparisons~5c and~5d corroborate these findings
using the broader non-GLOF reference group.

\begin{table}[H]
  \centering
  \caption{Rank-Sum test results (two-sided, $\alpha = 0.05$) for both discharge
           datasets. Comparisons~5a and~5b directly test $H_0^{(1)}$ and $H_0^{(2)}$
           against the pre-GLOF reference; comparisons~5c and~5d use all non-GLOF
           years as additional corroboration.}
  \label{tab:ranksum}
  \begin{tabular}{lllccl}
    \toprule
    Dataset & & Comparison & $W$ & $p$-value & Decision \\
    \midrule
    \multirow{4}{*}{BAFU 2219}
      & 5a & Pre-GLOF reference vs.\ GLOF-inclusive & 83.0  & 0.002 & Reject $H_0^{(1)}$ \\
    ...
    \midrule
    \multirow{4}{*}{GRDC}
      & 5a & Pre-GLOF reference vs.\ GLOF-inclusive & 65.0  & $<$0.001 & Reject $H_0^{(1)}$ \\
...
\begin{figure}[H]
  ...
  \caption{Left: 2011--2018 BAFU~2219 daily-maximum peaks on the reference GEV
           ...
           (Module~1, \S3.2). GLOF-inclusive lie far above the diagonal, driven by
           2018. GLOF-masked also lie systematically above the 1:1 line, indicating
           peaks slightly higher than the reference predicts; however, the shift does
           not reach statistical significance (KS $p = 0.195$; Rank-Sum 5b: $p = 0.172$).}
  \label{fig:pp}
\end{figure}
